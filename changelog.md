...
- Renamed from `demucs-next` to `unblend`: the Python distribution, module, CLI, and browser npm package are all `unblend` (`demucs` and `demucs-inference` remain as compatibility CLI aliases).
...
- All registered HTDemucs and RoFormer weights use pickle-free Safetensors. Architecture and constructor configuration live in the checked-in registry, model classes are selected from an allowlist, and exact byte sizes plus SHA-256 digests are verified before strict loading.
- Model download and availability checks support RoFormer's single-artifact registry entries as well as Demucs layer lists, with exact size limits and a bounded overall download time.
...
- ONNX export (`unblend export-onnx`) supports the RoFormer models, and the `unblend` npm package runs them in-browser (WebGPU/WASM) alongside HTDemucs.
