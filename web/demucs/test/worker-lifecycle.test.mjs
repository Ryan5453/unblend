...
for (const model of ['bs_roformer_sw', 'scnet_xl_wide_v5']) {
    test(`${model} refuses WASM before constructing workers`, async () => {
        await assert.rejects(
            Separator.load(model, { backend: 'wasm', precision: 'fp16' }),
            /requires WebGPU.*fixed memory heap/,
        );
        assert.equal(FakeWorker.instances.length, 0);
    });

    test(`${model} does not retry a failed WebGPU load on WASM`, async () => {
        Object.defineProperty(globalThis, 'navigator', {
            configurable: true,
            value: { gpu: { requestAdapter: async () => ({}) } },
        });
        ...
        await assert.rejects(
            Separator.load(model, { backend: 'webgpu', precision: 'fp16' }),
